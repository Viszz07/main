...
for u, v in A:
  ...
  # Uncomment the following line if the graph is undirected(both links)
  # M[v][u] = 1
...
while stack:
  ...
  print(node)
  for nei_node in D[node]:
    if nei_node not in new_seen:
      new_seen.add(nei_node)
...
while q:
  ...
  print(node)
  for nei_node in D[node]:
    if nei_node not in set_new:
      set_new.add(nei_node)
      q.append(nei_node)
